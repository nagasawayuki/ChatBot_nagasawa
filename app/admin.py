...
class MessageReviewView(SecureModelView):
    ...
    def export_csv(self, ids):  # ids=選択されたIdのリスト
        ...
        # ヘッダー行を作成
        writer.writerow(['Sender ID', 'User Message',
                        'AI Response', 'Review', 'Message Time'])

        for row in query.all():
            writer.writerow([row.sender_id, row.user_message,
                            row.ai_response, row.review, row.timestamp])
...
def init_admin(app):
    ...
    # messageとreviewの統合表示
    admin.add_view(MessageReviewView(Message, db.session))
